...
def extract(root, forms, radical_names, rules, joyo=None, radical_meanings=None,
    ...
    def children(elements, stop_at_radical):
        candidates = frontier(elements)
    ...
    def make(elements, stop_at_radical):
        ...
        if kind != 'radical' or not stop_at_radical:
...
def validate(tree, strokes):
    ...
    def visit(node):
        ...
        if children:
            assert Counter(s for c in children for s in c['stroke_ids']) == Counter(ids), node['id']
