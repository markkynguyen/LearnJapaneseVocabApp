...
select ok(
  exists (
    select 1
    from pg_constraint
    where conrelid = 'public.user_learning_settings'::regclass
      and conname = 'user_learning_settings_quiz_japanese_script_check'
      and pg_get_constraintdef(oid) like '%both%'
  ),
  'quiz Japanese script allows both kanji and kana'
);
select has_function(
  'public',
