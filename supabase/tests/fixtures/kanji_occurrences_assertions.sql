...
do $$ begin
  ...
  assert (select array_agg(display_form order by sort_order) from public.kanji_component_occurrences where kanji_id=ascii('機'))=array['木','幺','幺','戌'];
  ...
  assert not exists(select 1 from public.kanji_component_occurrences where display_form in ('戌','⺍') and radical_id is not null);
  assert not exists(select 1 from public.kanji_components where kanji_id=ascii('機') and component_form in ('戈','弋','丶'));
...
do $$ begin
  ...
  assert not exists(select 1 from public.user_radical_stats where radical_id=62), 'nested 戈 gone';
end $$;
reset role;
rollback;
